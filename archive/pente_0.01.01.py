...
class PenteAI:
    ...
    def MakeMove(self):
        ...
        B = 8    # move to the end of an opponent's open pair (try to trap)
        ...
        I = 10    # move to make a threesome (closed on one side) - prevents a trap
        J = 8    # move to make an open three out of a pair - can lead to a winner
                 # TO DO: Add ability to make an open three by moving between two singles
        K = 4    # move to make four in a row
        L = 25    # move to make an open four in a row
        M = 30   # move to make five in a row (win the game)
        N = 20    # move to fill in a gap in an open-ended run of four opponent pieces
        O = 25    # move to fill in a gap in a run of five opponent pieces
        P = 10    # move to block and open-three opportunity
        ...
        for run in m.Runs[HUMAN][2]:
            ...
            if len(endlist)==2: #ignore pairs at edge of board
              for pair in endlist:
                ...
                if DEBUG: print "found end of opponent's pair"
